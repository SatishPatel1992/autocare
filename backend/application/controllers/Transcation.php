...
class Transcation extends MY_Controller {
    ...
    public function InsertOperation() {
        if (isset($_REQUEST) && !empty($_REQUEST) && $_REQUEST['table_name'] != '') {
            ...
            if ($_REQUEST['table_name'] == 'tbl_garage') {
            ...
            } else if($_REQUEST['table_name'] == 'tbl_insurance') {
                unset($_REQUEST['table_name']);
                $insurance = $_REQUEST;
                $insurance['garage_id'] = $_SESSION['setting']->garage_id;
                $insurance['is_active'] = 1;
                $insurance['created_date'] = date('Y-m-d H:i:s');
                $insurance['updated_date'] = date('Y-m-d H:i:s');
                $this->InsertData('tbl_insurance', $insurance);
                ...
                $inventory['created_date'] = date('Y-m-d H:i:s');
                $this->InsertData('tbl_inventory', $inventory);
            } else if($_REQUEST['table_name'] == 'tbl_account_payment') {
                $payments = array();
                parse_str($_REQUEST['data'], $payments);
                
                $totalToBePaid = $payments['amount_paid'];

                if($payments['selectedBills']) {
                    $vendor_bill_ids = explode(',', $payments['selectedBills']);
                    $paidAdded = 0;
                    foreach ($vendor_bill_ids as $key => $value) {
                        $PO_Trans = explode('-',$value);
                        if($PO_Trans[0] == 'po') {
                            $vendor_bills = $this->db->query("select * from tbl_vendor_bills where tbl_vendor_bills.po_id = '".$PO_Trans[1]."'")->row_array();
                            $vpayment = $this->db->query("select sum(amount) as total_paid from tbl_payments where vendor_id = '".$vendor_bills['vendor_id']."' and item_id ='".$PO_Trans[1]."' and item_type='vendor_payment'")->row_array();
                            
                            if($totalToBePaid >= 0) {
                                $paymentDone = ($vpayment['total_paid'] != NULL ? $vpayment['total_paid'] : 0);
                                $amtPaid = $vendor_bills['grand_total'] - $paymentDone;

                                $bill_status = $vendor_bills['grand_total'] - (($totalToBePaid - $amtPaid > 0 ? $amtPaid : $totalToBePaid) + $paymentDone); 
                                if($totalToBePaid - $amtPaid > 0 ? $amtPaid : $totalToBePaid > 0) {
                                    $amountPaid = array();
                                    $amountPaid['garage_id'] = $_SESSION['setting']->garage_id;
                                    $amountPaid['item_id'] = $vendor_bills['po_id'];
                                    $amountPaid['item_type'] = 'vendor_payment';
                                    $amountPaid['customer_id'] = 0;
                                    $amountPaid['vendor_id'] = $vendor_bills['vendor_id'];
                                    $amountPaid['date'] = date('Y-m-d', strtotime($payments['date']));
                                    $amountPaid['payment_type_id'] = $payments['payment_type'];
                                    $amountPaid['amount'] = $totalToBePaid - $amtPaid > 0 ? $amtPaid : $totalToBePaid;
                                    $amountPaid['reference_no'] = '';
                                    $amountPaid['created_date'] = date('Y-m-d H:i:s');
                                    $this->InsertData('tbl_payments', $amountPaid);

                                    $this->db->where('po_id', $vendor_bills['po_id']);
                                    $this->db->update('tbl_vendor_bills',array('status' => $bill_status == 0 ? 'paid' : 'partial_paid'));
                                }
                            }
                        } else {
                            $trans_bills = $this->db->query("select * from tbl_transaction where transcation_id = '".$PO_Trans[1]."'")->row_array();
                            $vpayment = $this->db->query("select sum(amount) as total_paid from tbl_payments where vendor_id = '".$trans_bills['vendor_id']."' and item_id ='".$PO_Trans[1]."' and item_type='expense'")->row_array();
                            
                            if($totalToBePaid >= 0) {
                                $paymentDone = ($vpayment['total_paid'] != NULL ? $vpayment['total_paid'] : 0);
                                $amtPaid = $trans_bills['amount'] - $paymentDone;

                                $bill_status = $trans_bills['amount'] - (($totalToBePaid - $amtPaid > 0 ? $amtPaid : $totalToBePaid) + $paymentDone); 
                                if($totalToBePaid - $amtPaid > 0 ? $amtPaid : $totalToBePaid > 0) {
                                    $amountPaid = array();
                                    $amountPaid['garage_id'] = $_SESSION['setting']->garage_id;
                                    $amountPaid['item_id'] = $trans_bills['transcation_id'];
                                    $amountPaid['item_type'] = 'expense_payment';
                                    $amountPaid['customer_id'] = 0;
                                    $amountPaid['vendor_id'] = $trans_bills['vendor_id'];
                                    $amountPaid['date'] = date('Y-m-d', strtotime($payments['date']));
                                    $amountPaid['payment_type_id'] = $payments['payment_type'];
                                    $amountPaid['amount'] = $totalToBePaid - $amtPaid > 0 ? $amtPaid : $totalToBePaid;
                                    $amountPaid['reference_no'] = '';
                        ...
                        $totalToBePaid = $totalToBePaid - $amtPaid;
                    }
                    $status_code = 200;
                }
                ...
                $paid_amount = $transcation['paid'];
                unset($transcation['paid']);
                ...
                if($paid_amount != 0 && $paid_amount != '') {
                    $amountPaid = array();
                    $amountPaid['garage_id'] = $_SESSION['setting']->garage_id;
                    $amountPaid['item_id'] = $transcation_id;
                    $amountPaid['item_type'] = 'expense';
                    $amountPaid['customer_id'] = 0;
                    $amountPaid['vendor_id'] = $transcation['vendor_id'];
                    $amountPaid['date'] = date('Y-m-d');
                    $amountPaid['payment_type_id'] = 0;
                    $amountPaid['amount'] = $paid_amount;
                    $amountPaid['reference_no'] = '';
                    $amountPaid['created_date'] = date('Y-m-d H:i:s');
                    $this->InsertData('tbl_payments', $amountPaid);
                }
                ...
                if($_REQUEST['mainItem']['is_gst_bill'] == 'N') {
                    $invoiceCount = $this->db->query("select count(*) as total_invoice_count from tbl_invoices left join tbl_jobcard on tbl_jobcard.jobcard_id = tbl_invoices.item_id where tbl_invoices.garage_id = '".$_SESSION['setting']->garage_id."' and tbl_invoices.date between '".$fromDate."' and '".$toDate."' and is_gst_bill = 'N'")->row_array();
                    
                    if($invoiceCount['total_invoice_count'] == NULL || $invoiceCount['total_invoice_count'] == '') {
                        $next_invoice_no = $finYear.'/01';
                    } else {
                        $next_invoice_no = $finYear.'/'.(strlen($invoiceCount['total_invoice_count'] + 1) == 1 ? '0'.($invoiceCount['total_invoice_count'] + 1) : ($invoiceCount['total_invoice_count'] + 1));
                    }
                } else if($_REQUEST['mainItem']['is_gst_bill'] == 'Y') {
                    $invoiceCount = $this->db->query("select count(*) as total_invoice_count from tbl_invoices left join tbl_jobcard on tbl_jobcard.jobcard_id = tbl_invoices.item_id where tbl_invoices.garage_id = '".$_SESSION['setting']->garage_id."' and tbl_invoices.date between '".$fromDate."' and '".$toDate."' and is_gst_bill = 'Y'")->row_array();
                    
                    if($invoiceCount['total_invoice_count'] == NULL || $invoiceCount['total_invoice_count'] == '') {
                        $next_invoice_no = 'Inv/'.$finYear.'/01';
                    } else {
                        $next_invoice_no = 'Inv/'.$finYear.'/'.(strlen($invoiceCount['total_invoice_count'] + 1) == 1 ? '0'.($invoiceCount['total_invoice_count'] + 1) : ($invoiceCount['total_invoice_count'] + 1));
                    }
                }                
                ...
                if($_REQUEST['data']['invoice_status'] != 'payment_due') {
                    $dataArray['garage_id'] = $_SESSION['setting']->garage_id;
                    $dataArray['item_id'] = $inv_id;
                ...
                $this->db->select('tbl_invoices.invoice_no,tbl_invoices.amount as invoice_amount,tbl_invoices.item_id as jobcard_id');
                $this->db->from('tbl_invoices');
                $this->db->where('invoice_id',base64_decode($_REQUEST['data']['invoice_id']));
                $paid_details = $this->db->get()->row_array();
                ...
                // get po no.
                $this->db->select('max(CAST(po_no AS UNSIGNED)) as max_po');
                ...
                $po['po_no'] = $po_no;
                $po['invoice_date'] = $po['invoice_date'] != '' ? date('Y-m-d', strtotime($po['invoice_date'])) : '';
            ...
            if($_REQUEST['ticket_id'] == "") {
                // Get ticket no. 
                $this->db->select('max(CAST(ticket_no AS UNSIGNED)) as ticket_no');
                ...
                if($_REQUEST['action'] == 'order') {
                    ...
                    $this->db->select("max(CAST(invoice_no AS UNSIGNED)) as max_invoice_no");
                    $this->db->from("tbl_invoices");
                    $this->db->where("garage_id",$_SESSION['setting']->garage_id);
                ...
                if($_REQUEST['action'] != 'create_invoice' && $_REQUEST['action'] != 'add_payment') {
                        if($_REQUEST['action'] != "") {
                            $_REQUEST['mainItem']['status'] = $_REQUEST['action'] == 'create_invoice' ? 'payment_due' : $_REQUEST['action'] == 'job_done' ? 'invoice' : $_REQUEST['action'];
                        }
                        $_REQUEST['mainItem']['date'] = $_REQUEST['mainItem']['date'] != '' ? date('Y-m-d', strtotime($_REQUEST['mainItem']['date'])) : '';
                        $_REQUEST['mainItem']['expt_delivery_date'] = $_REQUEST['mainItem']['expt_delivery_date'] != '' ? date('Y-m-d', strtotime($_REQUEST['mainItem']['expt_delivery_date'])) : '';
                    ...
                    } else if($_REQUEST['action'] == 'job_done') {
                        $message = 'Jobcard completed successfully !';
                    } else {
            ...
            } else if($_REQUEST['table_name'] == 'tbl_make') {
                $makeData = array();
                $makeData['garage_id'] = $_SESSION['setting']->garage_id;
                $makeData['name'] = $_REQUEST['name'];
                $makeData['is_active'] = 1;
                $this->InsertData('tbl_make', $makeData);
                $status_code = 200;
            } else if($_REQUEST['table_name'] == 'tbl_model') {
                $modelData = array();
                $modelData['garage_id'] = $_SESSION['setting']->garage_id;
                $modelData['make_id'] = $_REQUEST['make_id'];
                $modelData['name'] = $_REQUEST['name'];
                $modelData['is_active'] = 1;
                $this->InsertData('tbl_model', $modelData);
                $status_code = 200;
            } else if($_REQUEST['table_name'] == 'tbl_variant') {
                $variantData = array();
                $variantData['garage_id'] = $_SESSION['setting']->garage_id;
                $variantData['model_id'] = $_REQUEST['model_id'];
                $variantData['name'] = $_REQUEST['name'];
                $variantData['is_active'] = 1;
                $this->InsertData('tbl_variant', $variantData);
                $status_code = 200;
                ...
                if($_REQUEST && $_REQUEST['data']) {
                    ...
                    if($_REQUEST['btnType'] == 1) { // receive order..
                     ...
                     $this->db->where('name','Credit Card');
            ...
            } else if($_REQUEST['table_name'] == 'tbl_category_master') {
                unset($_REQUEST['table_name']);
                if(!$_REQUEST['category_id']) {
                    ...
                    $this->InsertData('tbl_category', $category);
                } else {
                    $updateCat['name'] = $_REQUEST['name'];
                    $updateCat['head_type'] = $_REQUEST['head_type'];

                    $condition = array('category_id' => $_REQUEST['category_id']);
                    $this->UpdateData('tbl_category',$updateCat,$condition);
                }
                $status_code = 200;
                ...
                if($_REQUEST['action'] == 'insert') {
                    $category['garage_id'] = $_SESSION['setting']->garage_id;
                    $category['head_type'] = $_REQUEST['head_type'];
    ...
    public function UpdateOperation() {
        ...
        $conditionArray = array($_REQUEST['cond_field'] => $_REQUEST['cond_value']);
        unset($_REQUEST['table_name'],$_REQUEST['cond_field'],$_REQUEST['cond_value']);

        $DataArray = $_REQUEST;
        $this->db->where($conditionArray);
        $this->db->update($table_name, $DataArray);
        echo json_encode(array('status' => 200, 'message' => 'Updated Successfully !', 'data' => 1));
    }
